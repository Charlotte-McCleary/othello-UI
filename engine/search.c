...
float mobility_hueristic(bitboard opponent, bitboard player) {
    ...
    if (player_moves + opponent_moves == 0) {
        int player_disks = count_ones(player);
        int opponent_disks = count_ones(opponent);
        return (player_disks - opponent_disks) / ((float)(player_disks + opponent_disks));
    } else {
...
bitboard best_move(bitboard opponent, bitboard player, size_t depth) {
	...
	for (bitboard move = pop_ls1b(&moves); move; move = pop_ls1b(&moves)) {
        bitboard flipped = flip_all(opponent, player, move);
		float alphabeta_eval = alphabeta(player | flipped | move, opponent & ~flipped,
            false, depth, EVAL_MIN, EVAL_MAX);
        printf("%.4f\n", alphabeta_eval);
...
uint32_t best_move_pack32(uint32_t opponent_h, uint32_t opponent_l,
    ...
    bitboard move = best_move(((uint64_t)opponent_h << 32) | opponent_l,
                        ((uint64_t)player_h << 32) | player_l, depth);
    return count_ones((move & -move) - 1);
